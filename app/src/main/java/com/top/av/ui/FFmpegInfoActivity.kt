...
import com.top.ffmpeg.ffmpeg.FFmpegKitConfig
import com.top.ffmpeg.ffmpeg.Packages
...
public class FFmpegInfoActivity : BaseActivity<ActivityFfmpegInfoBinding>() {
    ...
    override fun init(root: View?) {
        mDataBinding.tvFfmpegVersion.text =
            String.format(resources.getString(R.string.ffmpeg_version, FFmpegKitConfig.getFFmpegVersion()))
    }

    fun Run(view: android.view.View) {
        val execute = FFmpegKit.execute("ffmpeg -v")
        mDataBinding.tvResult.text=execute.output
    }
