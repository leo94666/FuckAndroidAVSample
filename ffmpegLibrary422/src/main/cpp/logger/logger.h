...
class Logger
:
public std::enable_shared_from_this<Logger>, public
noncopyable {
public:
friend
        class
 ...
 * @param channel log通道
 */
void add(const std::shared_ptr <LogChannel>
 ...
 * @param writer 写log器
 */
void setWriter(const std::shared_ptr <LogWriter>
 ...
 * @param ctx 日志信息
 */
void write(const LogContextPtr
 ...
 * @param ctx 日志信息
 */
void writeChannels(const LogContextPtr
...
class LogContextCapturer{
        ...
        LogContextCapturer(Logger & logger, LogLevel
        level,
        const char *file,
        const char *function,
        int line);
        LogContextCapturer(
        const LogContextCapturer
        &that);
        ~LogContextCapturer();
        ...
        template < typename
        T > LogContextCapturer & operator << (T && data)
        {
            if (!_ctx) {
                return *this;
            }
...
public noncopyable{
        ...
        &name() const;
        void setLevel(LogLevel level);
        static std::string printTime(const timeval
        &tv);
...
class LoggerWrapper{
        ...
        template < typename
        First, typename... ARGS >
        static inline void printLogArray(Logger
        &logger, LogLevel
        level,
        const char *file,
        const char *function,
        ...
        Log, typename
        First, typename... ARGS >
        static inline void appendLog(Log
        &out, First && first, ARGS &&...args) {
...
//用法: DebugL << 1 << "+" << 2 << '=' << 3;
#define WriteL(level)                                                          \
  LogContextCapturer(getLogger(), level, __FILE__, __FUNCTION__, __LINE__)
...
#define PrintLog(level, ...)                                                   \
  LoggerWrapper::printLog(getLogger(), level, __FILE__, __FUNCTION__,          \
...
#define LogL(level, ...)                                                       \
  LoggerWrapper::printLogArray(getLogger(), (LogLevel)level, __FILE__,         \
