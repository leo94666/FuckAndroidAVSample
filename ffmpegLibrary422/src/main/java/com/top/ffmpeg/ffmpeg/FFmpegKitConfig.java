...
public class FFmpegKitConfig {
    ...
    private static void statistics(final long sessionId, final int videoFrameNumber,
        ...
        if (session != null && session.isFFmpeg()) {
            ...
            if (ffmpegSession.getStatisticsCallback() != null) {
                try {
                    ...
                    android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside session statistics callback.%s", ThrowableUtils.getStackTraceString(e)));
                }
            }
        }
        ...
        if (globalStatisticsCallbackFunction != null) {
            try {
                ...
                android.util.Log.e(FFmpegKitConfig.TAG, String.format("Exception thrown inside global statistics callback.%s", ThrowableUtils.getStackTraceString(e)));
            }
        }
    }
    ...
    public static void setFontDirectoryList(final Context context, final List<String> fontDirectoryList, final Map<String, String> fontNameMapping) {
        ...
        try {
            ...
            android.util.Log.e(TAG, String.format("Failed to set font directory: %s.%s", Arrays.toString(fontDirectoryList.toArray()), ThrowableUtils.getStackTraceString(e)));
        } finally {
    ...
    public static void ffmpegExecute(final FFmpegSession ffmpegSession) {
        ...
        try {
            ...
            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFmpeg execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffmpegSession.getArguments()), ThrowableUtils.getStackTraceString(e)));
        }
    }
    ...
    public static void ffprobeExecute(final FFprobeSession ffprobeSession) {
        ...
        try {
            ...
            android.util.Log.w(FFmpegKitConfig.TAG, String.format("FFprobe execute failed: %s.%s", FFmpegKitConfig.argumentsToString(ffprobeSession.getArguments()), ThrowableUtils.getStackTraceString(e)));
        }
    }
    ...
    static String extractExtensionFromSafDisplayName(final String safDisplayName) {
        ...
        try {
            ...
            android.util.Log.w(TAG, String.format("Failed to extract extension from saf display name: %s.%s", safDisplayName, ThrowableUtils.getStackTraceString(e)));
            return "raw";
        }
    }
    ...
    public static String getSafParameter(final Context context, final Uri uri, final String openMode) {
        ...
        try (Cursor cursor = context.getContentResolver().query(uri, null, null, null, null)) {
            ...
            android.util.Log.e(TAG, String.format("Failed to get %s column for %s.%s", DocumentsContract.Document.COLUMN_DISPLAY_NAME, uri.toString(), ThrowableUtils.getStackTraceString(t)));
            throw t;
        }
    ...
    private static int safOpen(final int safId) {
        try {
            ...
            android.util.Log.e(TAG, String.format("Failed to open SAF id: %d.%s", safId, ThrowableUtils.getStackTraceString(t)));
        }
    ...
    private static int safClose(final int fileDescriptor) {
        try {
            ...
            android.util.Log.e(TAG, String.format("Failed to close SAF fd: %d.%s", fileDescriptor, ThrowableUtils.getStackTraceString(t)));
        }
